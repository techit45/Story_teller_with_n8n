...
function loadConfig() {
  ...
  if (saved) {
    ...
    document.getElementById("webhookPath").value =
      config.webhookPath || "/webhook/storyteller";
  }
...
function saveConfig() {
  ...
  config.webhookPath = document
    .getElementById("webhookPath")
    .value.trim();
  ...
  addMessage(
    "bot",
    "✅ ตั้งค่าเรียบร้อยแล้ว! มาเริ่มสร้างนิทานกันเถอะ! 📚✨"
  );
}
...
async function sendMessage() {
  ...
  try {
    const response = await fetch(
      `${config.ngrokUrl}${config.webhookPath}`,
      {
        method: "POST",
        headers: {
